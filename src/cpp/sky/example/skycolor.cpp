...
//                *p = atmosphere.computeIncidentLight(Vec3f(0, atmosphere.earthRadius + 1, 0), dir, 0, kInfinity);
                *p = atmosphere.incident_light(glm::vec3(0, atmosphere.planet_radius + 1, 0), dir);
            }
        }
    ...
    glm::vec3 orig(0, atmosphere.planet_radius + 1000, 30000); // camera position
        ...
        for (unsigned x = 0; x < width; ++x, ++p) {
            for (unsigned m = 0; m < numPixelSamples; ++m) {
                for (unsigned n = 0; n < numPixelSamples; ++n) {
                    ...
                    if (intersectRaySphere(orig, dir, atmosphere.planet_radius, t0, t1) && t1 > 0)
                        tMax = std::max(0.f, t0);
                    // [comment]
